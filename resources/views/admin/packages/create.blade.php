                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-2 flex flex-col gap-2">
                            ...
                            <input type="text" name="name" id="name" placeholder="Paket Pernikahan Klasik Agung" value="{{ old('name') }}" class="w-full border-2 border-plum-ink rounded-xl px-4 py-2.5 font-sans text-sm focus:outline-none focus:ring-0 focus:border-sky-ribbon">
                            @error('name')
                        ...
                        <div class="md:col-span-1 flex flex-col gap-2">
                            ...
                            <select name="category_id" id="category_id" class="w-full border-2 border-plum-ink rounded-xl px-4 py-2.5 font-sans text-sm focus:outline-none focus:ring-0 focus:border-sky-ribbon">
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ old('category_id') == $cat->id || $cat->slug == 'paket-lengkap' ? 'selected' : '' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                                <p class="text-poppy-red font-mono text-[10px] mt-0.5">{{ $message }}</p>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <textarea name="description" id="description" rows="3" placeholder="Jelaskan isi paket sewa ini secara mendalam..." class="w-full border-2 border-plum-ink rounded-xl px-4 py-2.5 font-sans text-sm focus:outline-none focus:ring-0 focus:border-sky-ribbon">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-poppy-red font-mono text-[10px] mt-0.5">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="flex flex-col gap-2">
                            ...
                            <input type="number" name="base_price" id="base_price" placeholder="4500000" value="{{ old('base_price') }}" class="w-full border-2 border-plum-ink rounded-xl px-4 py-2.5 font-mono text-sm focus:outline-none focus:ring-0 focus:border-sky-ribbon">
                            @error('base_price')
                                <p class="text-poppy-red font-mono text-[10px] mt-0.5">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex flex-col gap-2">
                            <label for="cover_image" class="block font-mono text-xs uppercase font-bold tracking-wider text-plum-ink">Cover Gambar Paket</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-72 overflow-y-auto p-2 border-2 border-plum-ink/10 rounded-xl">
                        @foreach($items as $item)
                            <label class="flex items-center gap-3 p-3 bg-petal-cream/20 border border-plum-ink/10 rounded-lg cursor-pointer hover:bg-petal-cream/40 transition">
                                <input type="checkbox" name="items[]" value="{{ $item->id }}" {{ is_array(old('items')) && in_array($item->id, old('items')) ? 'checked' : '' }} class="rounded border-plum-ink text-marigold focus:ring-marigold">
                                <div class="text-xs">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                        @foreach($services as $srv)
                            <div class="flex items-center justify-between p-3 border border-plum-ink/10 rounded-xl bg-white shadow-sm" x-data="{ selected: false }">
                                <label class="flex items-center gap-3 cursor-pointer">
                                    <input type="checkbox" name="services[]" value="{{ $srv->id }}" @change="selected = $el.checked" {{ is_array(old('services')) && in_array($srv->id, old('services')) ? 'checked' : '' }} class="rounded border-plum-ink text-sky-ribbon focus:ring-sky-ribbon">
                                    <div>
                                ...
                                <div class="flex items-center gap-2" x-show="selected" x-transition>
                                    <input type="checkbox" name="default_services[]" value="{{ $srv->id }}" id="def_{{ $srv->id }}" {{ is_array(old('default_services')) && in_array($srv->id, old('default_services')) ? 'checked' : '' }} class="rounded border-plum-ink text-meadow-green focus:ring-meadow-green">
